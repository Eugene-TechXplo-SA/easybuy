...
import type { ApiCategory, Product } from "@/types/product";
...
const ShopWithSidebar = () => {
  ...
  useEffect(() => {
    ...
    fetchProducts({
      ...
      .then((res) => {
        setProducts(res.products.map(apiProductToProduct));
        setTotal(res.total);
                ...
                <div className="flex flex-col gap-6">
                  <div className="bg-white shadow-1 rounded-lg py-4 px-5">
                    <div className="flex items-center justify-between">
                      <p>Filters:</p>
                      <button onClick={handleClearFilters} className="text-blue">
                        Clear All
                      </button>
                    </div>
                  </div>
