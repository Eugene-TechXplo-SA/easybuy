...
export default function PromotionsTable() {
  ...
  const openEdit = (p: Promotion) => {
    setSelected(p);
    setForm({ title: p.title, subtitle: p.subtitle, discount_label: p.discount_label, description: p.description, image_url: p.image_url, button_text: p.button_text, button_link: p.button_link, is_active: p.is_active, sort_order: p.sort_order });
    setModal("edit");
  };
  ...
  const handleToggleActive = async (p: Promotion) => {
    try {
      await patch(p.id, { is_active: !p.is_active });
      load();
    } catch {
    ...
    { header: "Sort", render: (p: Promotion) => <span>{p.sort_order}</span> },
    {
      ...
      render: (p: Promotion) => (
        <button onClick={() => handleToggleActive(p)} className={`relative inline-flex h-5 w-9 rounded-full transition-colors ${p.is_active ? "bg-green-500" : "bg-gray-200"}`}>
          <span className={`inline-block h-4 w-4 rounded-full bg-white shadow transform transition-transform mt-0.5 ${p.is_active ? "translate-x-4.5" : "translate-x-0.5"}`} />
        </button>
      ),
    },
    {
  ...
  return (
    <div>
      ...
      {(modal === "create" || modal === "edit") && (
        <AdminModal title={modal === "create" ? "Add Promotion" : "Edit Promotion"} onClose={() => setModal(null)} size="lg"
          footer={<>
      ...
      {modal === "delete" && selected && (
        <AdminModal title="Delete Promotion" onClose={() => setModal(null)}
          footer={<>
