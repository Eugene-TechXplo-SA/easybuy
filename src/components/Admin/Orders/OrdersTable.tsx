...
interface OrderItem {
  ...
  quantity: number;
  unit_price: number;
  products: { title: string; thumbnail_images: string[] } | null;
}
...
export default function OrdersTable() {
  ...
  const columns = [
    { header: "Order ID", render: (o: Order) => <span className="font-mono text-xs text-gray-500">{o.id.slice(0, 8)}…</span>, className: "w-28" },
    {
      ...
      render: (o: Order) => (
        <div>
          <p className="font-medium text-gray-900">{o.billing_first_name} {o.billing_last_name}</p>
          <p className="text-xs text-gray-500">{o.billing_email}</p>
        </div>
      ),
    },
      ...
      {editOrder && (
        <AdminModal
          ...
          footer={<>
            ...
            <button onClick={handleStatusUpdate} disabled={submitting} className="px-4 py-2 text-sm font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700 disabled:opacity-50">
              {submitting ? "Saving..." : "Update"}
            </button>
          </>}
        >
          <div className="space-y-3">
            <p className="text-sm text-gray-600">Order <strong>{editOrder.id.slice(0, 8)}…</strong> — current status: <strong>{editOrder.status}</strong></p>
            <div>
