...
export default async function AdminDashboardPage() {
  ...
  const [ordersCount, productsCount, customersCount, revenueResult, recentOrdersResult] = await Promise.all([
    supabase.from("orders").select("*", { count: "exact", head: true }),
    supabase.from("products").select("*", { count: "exact", head: true }),
    supabase.from("user_profiles").select("*", { count: "exact", head: true }),
    supabase.from("orders").select("total").eq("status", "delivered"),
    supabase.from("orders").select("id, status, total, billing_first_name, billing_last_name, billing_email, created_at").order("created_at", { ascending: false }).limit(8),
  ]);

  const totalRevenue = ((revenueResult.data ?? []) as { total: number }[]).reduce((sum, o) => sum + (o.total ?? 0), 0);
  ...
  const quickLinks = [
    ...
    { label: "View Store", href: "/", desc: "Open the public storefront" },
  ];
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
        {stats.map((s) => <StatCard key={s.label} label={s.label} value={s.value} subLabel={(s as { subLabel?: string }).subLabel} icon={s.icon} />)}
      </div>
